...
// We also accept the plain ASCII " and smart-quote variants.

const PRICE_RE  = /₪\s*([\d,]+)|([\d,]+)\s*(?:₪|ש[״"'`]?ח|שח|שקל(?:ים)?)|שכ[״"'`""׳]?ד\s*([\d,]+)/g;
...
// true:  דמי תיווך | עמלת תיווך | מתיווך | שיווק נדל"ן (real-estate agency tag)
// null:  anything else
...
const BROKER_RE    = /דמי\s+תיווך|עמלת\s+תיווך|מתיווך|שיווק\s+נדל[״"'""׳]?ן/;
