...
const PRICE_RE  = /₪\s*([\d,.]+)|([\d,.]+)\s*(?:₪|ש[״"'`׳]{0,2}ח|שח|שקל(?:ים)?|לחודש|ללא|לחדר)|שכ[״"'`""׳]?ד\s*([\d,.]+)|לחודש\s*([\d,.]+)|שכר\s+דירה\s*([\d,.]+)|מחיר\s*[:\-]?\s*(?:מבוקש\s*[:\-]?\s*)?([\d,.]+)|([\d,.]+)\s*כולל|ב[-־]\s*([\d,.]+)\s*!|החל\s+מ[-־]?\s*([\d,.]+)/g;
...
const RENTAL_RE = /להשכרה|לשכירות|שכירות|להשכיר|שכ[״"'`׳]?ד|שכר\s+דירה|דמי\s+שכירות|מתפנ(?:ה|ת|ים|ות)\s+(?:\d+\s+)?(?:דירה|חדר|יחידה)|מתפנ(?:ה|ת|ים|ות)\s+(?:ב[-]?\s*)?\d|מפנ(?:ה|ים|ות)\s+(?:את\s+)?ה?(?:דירה|בית|חדר)|מחפש(?:ת|ים)?\s+מחליפ|מחפש(?:ת|ות|ים)?\s+שות[פף]|מחפש(?:ת|ות|ים)?\s+(?:שוכר|דייר)|דירת\s+(?:\d+\s+|(?:שניים|שתיים|שני|שתי|שלושה|שלוש|ארבעה|ארבע|חמישה|חמש)\s+)?שותפ|סאבלט|כניסה\s*[-:]?\s*(?:מיידית|(?:ב[-]?\s*)?\d+[./]|לחדר)|משכיר(?:ים|ות)?|תי?ווך|לטווח\s+ארוך|עבור\s+בעל(?:ת)?\s+הדירה|\d[\d,]{3,}\s*כולל|(?<!\d)(?:[1-9]|[1-3]\d)\s*אלף(?!\s*ק)|מחיר\s*[:\-]?\s*(?:מבוקש\s*[:\-]?\s*)?(?:[1-9],?\d{3}|[1-3]\d,?\d{3})(?![\d,])|₪\s*\d{1,2},?\d{3}.{0,80}·.{0,80}דיר|for\s+rent(?:al)?(?:\s|$)|available\s+(?:for|from)|monthly\s+rent|long[\s-]?term\s+rent(?:al)?|entry\s+date|real\s+estate\s+agent|israeli\s+real\s+estate|apartment\/condo/i;
...
const SEEKER_RE = /מחפש(?:ת|ות|ים)?\s+(?:אחר\s+)?(?:דירה|דירת|דירות|סאבלט|יחיד(?:ה|ת))|looking\s+for\s+(?:an?\s+)?(?:apartment|flat|sublet|studio|room\b)/i;
...
const STRONG_RENTAL_RE = /להשכרה|לשכירות|להשכיר|שכ[״"'`׳]?ד|שכר\s+דירה|דמי\s+שכירות|מחפש(?:ת|ות|ים)?\s+(?:שוכר|דייר)|for\s+rent|monthly\s+rent/i;
...
export function regexClassifyPost(text) {
  ...
  // 3. Unambiguous offer language.
  if (STRONG_RENTAL_RE.test(text) || MONTHLY_PRICE_RE.test(text)) return 'rental';
  // 4. Wanted ads (apartment seekers) — checked after strong offers because
  //    broker listings open with rhetorical "מחפשים דירה?" hooks. A question
  //    mark shortly after the מחפש phrase marks such a hook ("מחפשים דירה
  //    נעימה...? הדירה הזו בדיוק בשבילכם!") — real seekers state, not ask.
