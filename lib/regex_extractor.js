...
const PRICE_RE  = /₪\s*([\d,.]+)|([\d,.]+)\s*(?:₪|ש[״"'`׳’”“]{0,2}ח|שח|שקל(?:ים)?|לחודש|ללא|לחדר|לשותף|פלוס)|שכ[״"'`""׳]?ד\s*([\d,.]+)|לחודש\s*([\d,.]+)|שכר\s+דירה\s*([\d,.]+)|מחיר\s*[:\-]?\s*(?:מבוקש\s*[:\-]?\s*)?([\d,.]+)|([\d,.]+)\s*כולל|ב[-־]\s*([\d,.]+)\s*!|החל\s+מ[-־]?\s*([\d,.]+)/g;
...
const LABELED_PRICE_RE = /(?:שכ[״"'`""׳]?ד|שכר\s+ה?דירה|דמי\s+שכירות|שכירות|מחיר\s+מבוקש)\s*[:\-–]?\s*(?:(?:מבוקש|הרצוי)\s*[:\-]?\s*)?כ?([\d,.]+)|rent\s*:?\s*([\d,.]+)/gi;
...
// English: "3 rooms" / "3 beds" / "Two bedrooms" — Marketplace cards & English posts
...
const NO_BROKER_RE = /ללא\s+(?:כל\s+)?(?:דמי\s+|עמלת\s+)?תיווך|ללא\s+מתווכ(?:ים)?|אין\s+(?:דמי\s+|עמלת\s+)?תיווך|בלי\s+(?:דמי\s+|עמלת\s+)?תיווך|לא\s+(?:דמי\s+|עמלת\s+)?מ?תי?ווך|ישיר(?:ות)?\s+מ(?:ה?בעל|ה?דייר|ה?משכיר)|פרטי\s+(?:מ(?:בעל|משכיר|דייר))|מפרטי(?![א-ת])(?!\s+ה)|no\s+(?:broker|fee|commission|agency)|owner\s+only/i;
const BROKER_RE    = /דמי\s+תיווך|עמלת\s+תיווך|מתיווך|מתווכ(?:ים|ת)?|מתווך|שיווק\s+נדל[״"'""׳]?ן|תיווך|תווך|מס(?:פר)?['׳]?\s*רישיון|רישיון\s*(?:מס(?:פר)?['׳]?)?\s*[:#]?\s*\d{4,}|listed\s+via\s+agency|real\s+estate|realty/i;
...
const ROOMMATES_RE = /(?:מחפש(?:ת|ות|ים)?|דרוש(?:ה)?|מוצע)\s+שות|מחפש(?:ת|ות|ים)?\s+מחליפ|דיר(?:ת|ות)\s+(?:\d+\s+|(?:שניים|שתיים|שני|שתי|שלושה|שלוש|ארבעה|ארבע|חמישה|חמש)\s+)?שותפ|שותפ(?:ים|ף|ה)?\s+ל(?:דירה|חדר)|נשאר(?:ת|ות|ים)?\s+(?:בדירה\s+)?(?:\d+\s+|(?:שניים|שתיים|שני|שתי|שלושה|שלוש)\s+)?שות|להי?כנס\s+שות|roommate\s+(?:wanted|needed|sought)|looking\s+for\s+(?:a\s+)?roommate|flatmate/i;
...
const DATE_MONTH_ENTRY_RE = new RegExp(
  '(?:כניסה\\s+(?:ב(?:חודש\\s+)?)?|פינוי\\s+ב(?:חודש\\s+)?|מתפנ(?:ה|ת|ים|ות)\\s+ב|שייכנסו\\s+לחוזה\\s+ב)(' + Object.keys(HEB_MONTHS).join('|') + ')(?:\\s+(\\d{4}))?',
  'g'
);
...
const RENTAL_RE = /ל?השכרה|לשכירות|שכירות|להשכיר|(?<![א-ת])שכ[״"'`׳’”]?ד(?![א-ת])|שכר\s+ה?דירה|שכר\s+חודשי|דמי\s+שכירות|מתפנ(?:ה|ת|ים|ות)\s+(?:\d+\s+)?(?:דירה|חדר|יחידה)|מתפנ(?:ה|ת|ים|ות)\s+(?:ב[-]?\s*)?\d|מפנ(?:ה|ים|ות)\s+(?:את\s+)?ה?(?:דירה|בית|חדר)|מחפש(?:ת|ים)?\s+מחליפ|מחפש(?:ת|ות|ים)?\s+שות[פף]|מחפש(?:ת|ות|ים)?\s+(?:שוכר|דייר)|דיר(?:ת|ות)\s+(?:\d+\s+|(?:שניים|שתיים|שני|שתי|שלושה|שלוש|ארבעה|ארבע|חמישה|חמש)\s+)?שותפ|ס(?:א)?בלט|כניסה\s*[-:]?\s*(?:מיידית|(?:ב[-־]?\s*)?\d+[./]|לחדר|לחוזה)|משכיר(?:ים|ות)?|תי?ווך|(?:לגור|לשכור|שכירות|חוזה|ל?השכרה|דירה)\s+לטווח\s+ארוך|עבור\s+בעל(?:ת)?\s+הדירה|\d[\d,]{3,}\s*כולל|(?<!\d)(?:[1-9]|[1-3]\d)\s*אלף(?!\s*(?:ק|עוקב))|מחיר\s*[:\-]?\s*(?:מבוקש\s*[:\-]?\s*)?(?:[1-9],?\d{3}|[1-3]\d,?\d{3})(?![\d,])|₪\s*\d{1,2},?\d{3}.{0,80}·.{0,80}דיר|מ?ה?\d{1,2}[./]\d{1,2}\s*עד\s*ה?\d{1,2}[./]\d{1,2}|for\s+rent(?:al)?(?:\s|$)|available\s+(?:from|now|immediately)|available\s+for\s+(?!tonight|today|this\s)|monthly\s+rent|long[\s-]?term\s+rent(?:al)?|entry\s+date|real\s+estate\s+agent|israeli\s+real\s+estate|apartment\/condo|аренд|сда[её]тся|сда[юм]|מחפש\S{0,3}\s+מישה[וי]\s+שיגור/i;
...
const NOT_RENTAL_RE  = /למכירה|למכור|נמכר(?:ת|ים|ות)?(?:\s|$)|להשקעה|למשקיעים|for\s+sale(?:\s|$)|(?<![א-ת])קני[יה]ה?(?:\s|$)|לקנות\s+(?:דירה|נכס|בית)/i;
...
const MILLION_PRICE_RE = /\d{1,2}[.,]\d{3}[.,]\d{3}|\d+(?:[.,]\d+)?\s*(?:מיליון|מליון|million)/i;
...
const SEEKER_RE = /(?<!אם\s{1,3}את(?:ם|ן|ה)?\s{1,3})(?<!ש)מחפש(?:ת|ות|ים)?\s+(?:אחר\s+)?(?:את\s+)?(?:דירה|דירת|דירות|סאבלט|יחיד(?:ה|ת)|\d+(?:[.,]\d+)?\s*חדרים)|מעוניי(?:ן|נת|נים|נות)\s+להי?כנס\s+לדיר|looking\s+for\s+(?:an?\s+)?(?:apartment|flat|sublet|studio|room\b)/i;
...
const STRONG_RENTAL_RE = /ל?השכרה|לשכירות|להשכיר|(?<![א-ת])שכ[״"'`׳’”]?ד(?![א-ת])|שכר\s+ה?דירה|שכר\s+חודשי|דמי\s+שכירות|מחפש(?:ת|ות|ים)?\s+(?:שוכר|דייר)|for\s+rent|monthly\s+rent/i;
