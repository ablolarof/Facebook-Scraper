...
const RENTAL_RE = /להשכרה|לשכירות|שכירות|להשכיר|שכ[״"'`׳]?ד|מתפנ(?:ה|ת|ים|ות)\s+(?:\d+\s+)?(?:דירה|חדר|יחידה)|מפנ(?:ה|ים|ות)\s+(?:את\s+)?ה?(?:דירה|בית|חדר)|מחפש(?:ת|ים)?\s+מחליפ|מחיר\s+(?:מבוקש\s+)?[\d,]{4,}|for\s+rent(?:al)?(?:\s|$)|available\s+for\s+rent/i;
