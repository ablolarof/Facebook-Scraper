...
const PRICE_RE  = /₪\s*([\d,.]+)|([\d,.]+)\s*(?:₪|ש[״"'`׳]{0,2}ח|שח|שקל(?:ים)?)|שכ[״"'`""׳]?ד\s*([\d,.]+)|לחודש\s*([\d,.]+)|שכר\s+דירה\s*([\d,.]+)/g;
...
const SIZE_RE  = /(\d+)\s*(?:מ[״׳"'""'″]{1,2}ר|מטר(?:\s+רבוע)?(?!\s+\d)|sqm)/gi;
const SIZE_MIN = 10;
const SIZE_MAX = 1000;
...
const NO_BROKER_RE = /ללא\s+(?:כל\s+)?(?:דמי\s+|עמלת\s+)?תיווך|אין\s+(?:דמי\s+|עמלת\s+)?תיווך|בלי\s+(?:דמי\s+|עמלת\s+)?תיווך|ישיר(?:ות)?\s+מ(?:ה?בעל|ה?דייר|ה?משכיר)|פרטי\s+(?:מ(?:בעל|משכיר|דייר))|no\s+(?:broker|fee|commission)|owner\s+only/i;
const BROKER_RE    = /דמי\s+תיווך|עמלת\s+תיווך|מתיווך|שיווק\s+נדל[״"'""׳]?ן|תיווך/;
...
const ROOMMATES_RE = /(?:מחפש|מחפשת|דרוש|דרושה|מוצע)\s+שות|דירת\s+(?:\d+\s+)?שותפ|שותפ(?:ים|ף|ה)?\s+ל(?:דירה|חדר)|נשאר(?:ת|ים)?\s+(?:\d+\s+)?שות|roommate\s+(?:wanted|needed|sought)|looking\s+for\s+(?:a\s+)?roommate|flatmate/i;
...
// Day/month without year after כניסה: "כניסה 15/6" → infer current or next year.
// Negative lookahead prevents matching when a year component follows (DATE_SLASH_RE covers that).
const DATE_NO_YEAR_RE = /כניסה\s+(\d{1,2})[./](\d{1,2})(?![./\d])/g;
...
// מתפנה/מתפנת/מתפנים/מתפנות + דירה/חדר = "a unit is becoming available"
const RENTAL_RE = /להשכרה|לשכירות|שכירות|להשכיר|מתפנ(?:ה|ת|ים|ות)\s+(?:דירה|חדר)|for\s+rent(?:al)?(?:\s|$)|available\s+for\s+rent/i;
