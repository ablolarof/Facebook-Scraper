...
const RENTAL_RE = /להשכרה|לשכירות|שכירות|להשכיר|שכ[״"'`׳]?ד|מתפנ(?:ה|ת|ים|ות)\s+(?:\d+\s+)?(?:דירה|חדר|יחידה)|מתפנ(?:ה|ת|ים|ות)\s+\d|מפנ(?:ה|ים|ות)\s+(?:את\s+)?ה?(?:דירה|בית|חדר)|מחפש(?:ת|ים)?\s+מחליפ|מחפש(?:ת|ות|ים)?\s+דיר|סאבלט|כניסה\s+(?:מיידית|\d+[./])|משכיר(?:ים|ות)?|מתיווך|לטווח\s+ארוך|מחיר[:\s]+(?:מבוקש[:\s]+)?[\d,]{4,}|for\s+rent(?:al)?(?:\s|$)|available\s+for\s+rent|monthly\s+rent|long[\s-]?term\s+rent(?:al)?/i;
