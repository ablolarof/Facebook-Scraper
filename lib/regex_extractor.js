...
const LABELED_PRICE_RE = /(?:שכ[״"'`""׳]?ד|שכר\s+ה?דירה|דמי\s+שכירות|שכירות)\s*[:\-–]?\s*(?:(?:מבוקש|הרצוי)\s*[:\-]?\s*)?כ?([\d,.]+)|rent\s*:?\s*([\d,.]+)/gi;
...
const ROOMMATES_RE = /(?:מחפש(?:ת|ות|ים)?|דרוש(?:ה)?|מוצע)\s+שות|מחפש(?:ת|ות|ים)?\s+מחליפ|דיר(?:ת|ות)\s+(?:\d+\s+|(?:שניים|שתיים|שני|שתי|שלושה|שלוש|ארבעה|ארבע|חמישה|חמש)\s+)?שותפ|שותפ(?:ים|ף|ה)?\s+ל(?:דירה|חדר)|נשאר(?:ת|ים)?\s+(?:בדירה\s+)?(?:\d+\s+)?שות|roommate\s+(?:wanted|needed|sought)|looking\s+for\s+(?:a\s+)?roommate|flatmate/i;
...
const RENTAL_RE = /להשכרה|לשכירות|שכירות|להשכיר|שכ[״"'`׳]?ד|שכר\s+ה?דירה|שכר\s+חודשי|דמי\s+שכירות|מתפנ(?:ה|ת|ים|ות)\s+(?:\d+\s+)?(?:דירה|חדר|יחידה)|מתפנ(?:ה|ת|ים|ות)\s+(?:ב[-]?\s*)?\d|מפנ(?:ה|ים|ות)\s+(?:את\s+)?ה?(?:דירה|בית|חדר)|מחפש(?:ת|ים)?\s+מחליפ|מחפש(?:ת|ות|ים)?\s+שות[פף]|מחפש(?:ת|ות|ים)?\s+(?:שוכר|דייר)|דיר(?:ת|ות)\s+(?:\d+\s+|(?:שניים|שתיים|שני|שתי|שלושה|שלוש|ארבעה|ארבע|חמישה|חמש)\s+)?שותפ|ס(?:א)?בלט|כניסה\s*[-:]?\s*(?:מיידית|(?:ב[-]?\s*)?\d+[./]|לחדר)|משכיר(?:ים|ות)?|תי?ווך|לטווח\s+ארוך|עבור\s+בעל(?:ת)?\s+הדירה|\d[\d,]{3,}\s*כולל|(?<!\d)(?:[1-9]|[1-3]\d)\s*אלף(?!\s*(?:ק|עוקב))|מחיר\s*[:\-]?\s*(?:מבוקש\s*[:\-]?\s*)?(?:[1-9],?\d{3}|[1-3]\d,?\d{3})(?![\d,])|₪\s*\d{1,2},?\d{3}.{0,80}·.{0,80}דיר|מ?ה?\d{1,2}[./]\d{1,2}\s*עד\s*ה?\d{1,2}[./]\d{1,2}|for\s+rent(?:al)?(?:\s|$)|available\s+(?:for|from)|monthly\s+rent|long[\s-]?term\s+rent(?:al)?|entry\s+date|real\s+estate\s+agent|israeli\s+real\s+estate|apartment\/condo/i;
...
const NOT_RENTAL_RE  = /למכירה|למכור|נמכר(?:ת|ים|ות)?(?:\s|$)|להשקעה|למשקיעים|for\s+sale(?:\s|$)|קני[יה]ה?(?:\s|$)|לקנות\s+(?:דירה|נכס|בית)/i;
const NOT_RENTAL_RE2 = /מוכר(?:ת|ים|ות)?\s+(?:דירה|נכס|בית|דו-?משפחתי)/i;

// Million-scale comma-formatted price (2,450,000) — a sale, never rent.
...
const MILLION_PRICE_RE = /\d{1,2},\d{3},\d{3}/;
...
const SEEKER_RE = /(?<!אם\s{1,3}את(?:ם|ן|ה)?\s{1,3})(?<!ש)מחפש(?:ת|ות|ים)?\s+(?:אחר\s+)?(?:דירה|דירת|דירות|סאבלט|יחיד(?:ה|ת))|looking\s+for\s+(?:an?\s+)?(?:apartment|flat|sublet|studio|room\b)/i;
...
const STRONG_RENTAL_RE = /להשכרה|לשכירות|להשכיר|שכ[״"'`׳]?ד|שכר\s+ה?דירה|שכר\s+חודשי|דמי\s+שכירות|מחפש(?:ת|ות|ים)?\s+(?:שוכר|דייר)|for\s+rent|monthly\s+rent/i;
