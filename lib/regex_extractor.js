...
// ── Neighborhood ──────────────────────────────────────────────────────────────
//
// Applies NEIGHBORHOOD_OVERRIDES against the post text. Any match is stored
// with confidence: "high". No match → all three fields stay null.
...
const ROOMMATES_RE = /(?:מחפש|מחפשת|דרוש|דרושה|מוצע)\s+שות|דירת\s+שותפ|שותפ(?:ים|ף|ה)?\s+ל(?:דירה|חדר)|נשאר(?:ת|ים)?\s+(?:\d+\s+)?שות|roommate\s+(?:wanted|needed|sought)|looking\s+for\s+(?:a\s+)?roommate|flatmate/i;
