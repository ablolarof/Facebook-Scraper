...
const PRICE_RE  = /₪\s*([\d,.]+)|([\d,.]*\d)\s*(?:₪|ש[״"'`׳’”“]{0,2}ח|שח|שקל(?:ים)?|לחודש|ללא|לחדר|לשותף|פלוס|לא\s+כולל|shekels?|nis\b)|שכ[״"'`""׳]?ד\s*([\d,.]+)|לחודש\s*([\d,.]+)|שכר\s+דירה\s*([\d,.]+)|מחיר\s*[:\-]?\s*(?:(?:מבוקש|מציאה|החדר|הדירה)\s*[!:\-]*\s*)?([\d,.]*\d)|([\d,.]+)\s*כולל|ב[-־]\s*([\d,.]+)\s*!|החל\s+מ[-־]?\s*([\d,.]+)|([\d,.]*\d)\s*שכר\s+דירה|([\d,.]*\d)\s+מחיר\s+שיווק|([\d,.]*\d)\s*,\s*כניסה|([\d,.]*\d)\s*\+\s*(?:הוצאות|חשבונות)|ב[-־](?!20\d\d(?!\d))([1-9]\d?,?\d{3})(?![\d,.])/g;
...
const LABELED_PRICE_RE = /(?:שכ[״"'`""׳]?ד|שכר\s+ה?דירה|דמי\s+שכירות|שכירות|מחיר\s+מבוקש)\s*[:\-–]?\s*(?:(?:מבוקש|הרצוי|חודשי)\s*[:\-]?\s*)?כ?([\d,.]*\d)|(?:monthly\s+)?(?:rent|price)\s*:\s*([\d,.]*\d)/gi;
