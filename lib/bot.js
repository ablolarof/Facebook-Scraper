...
async function handleMessage(msg) {
  ...
  if (cmd === '/retrain') {
    ...
    try {
      ...
      // Shadow heads ride the same command. They never tag, so this is
      // reported separately and can't be confused with the classifier above.
      // Stored predictions are now stale — re-scoring them needs the dashboard
      // (🔬 Shadow Backfill / Re-test Regex + ML), so say so rather than
      // leaving a silently outdated review queue.
      if (r.shadow) {
        const lines = Object.entries(r.shadow).map(([name, s]) => s.promoted
          ? `🔬 ${name}: promoted (held-out ${(s.score * 100).toFixed(1)}%, was ${(s.previous * 100).toFixed(1)}%)`
          : `🔬 ${name}: not promoted — ${s.reason}`);
        if (Object.values(r.shadow).some(s => s.promoted)) {
          lines.push('These predict but never tag. Open the dashboard and run 🔬 Shadow Backfill to re-score stored posts with the new weights.');
        }
