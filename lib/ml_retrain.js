...
import { buildCorpus, trainLR, crossValProbs, exportHead, predictRow, mulberry32 } from './ml_train_core.js';
import { NO_BROKER_RE, BROKER_RE, regexExtractTags } from './regex_extractor.js';
import { FEATURE_VERSION } from './ml_features.js';
import {
  buildPriceTrainingRows, priceCandidates, priceFeatures, isShortStayListing,
  PRICE_FEATURE_VERSION, PRICE_CONFIDENCE, PRICE_STORAGE_KEY,
} from './ml_price.js';
import {
  maskRoommateKeywords, ROOMMATES_FEATURE_VERSION, ROOMMATES_STORAGE_KEY,
} from './ml_roommates.js';
import { verdictTruth } from './ml_shadow.js';
...
// ── Shadow heads (price, roommates) ─────────────────────────────────────────
//
// Same button, same corrections, no Node. These are trained here rather than
// only in ml/train_price.mjs + ml/train_roommates.mjs so the whole loop —
// judge a disagreement, retrain, watch the score move — lives in the
// extension. The featurizers are imported from lib/ml_price.js and
// lib/ml_roommates.js, exactly the modules the runtime scores with, so the
// offline and in-extension trainers cannot drift apart.
//
// TWO DIFFERENT BARS, do not confuse them:
//
//   * The gate below is a REGRESSION GUARD. It holds out 20% of the posts
//     whose answer we already know and checks the new weights still reproduce
//     them. It answers "did this retrain break the model?".
//   * The 96% / beat-the-regex bar lives in ml_shadow.js::scoreShadow and is
//     measured on YOUR VERDICTS. It answers "is this head good enough to
//     start tagging with?" — which is a separate, later decision this
//     function never makes.
//
// A head that clears the gate is still shadow-only. Nothing here promotes a
// field into real tags.

const PRICE_BASELINE     = 0.97;   // held-out top-1 precision of the shipped weights (measured 99.4%)
const ROOMMATES_BASELINE = 0.93;   // held-out accuracy on masked text (measured 96.2%)
const SHADOW_GATE_EPSILON = 0.01;

const rentalPosts = posts => posts.filter(p =>
  (p.human_label || p.ai_label) === 'rental' && !p.is_duplicate && (p.text || '').trim());

// Label precedence for both heads: shadow VERDICT > tags_human_override > regex.
// The verdict is read explicitly rather than with `??` because a verdict of
// null is itself an answer ("this post states no monthly rent") and `??` would
// discard it in favour of the regex.
function priceLabelFor(post) {
  const vt = verdictTruth(post, 'price');
  if (vt !== undefined) return { price: vt, confirmedAbsent: vt === null, src: 'verdict' };
  const o = post.tags_human_override;
  if (o && Object.prototype.hasOwnProperty.call(o, 'price')) {
    return { price: o.price ?? null, confirmedAbsent: false, src: 'override' };
  }
  return { price: (regexExtractTags(post.text) || {}).price ?? null, confirmedAbsent: false, src: 'regex' };
}

function trainPriceHead(posts, rng = mulberry32(41)) {
  const known = [], absent = [];
  let fromHuman = 0;
  for (const p of rentalPosts(posts)) {
    const { price, confirmedAbsent, src } = priceLabelFor(p);
    if (src !== 'regex') fromHuman++;
    if (price != null)        known.push({ text: p.text, price });
    else if (confirmedAbsent) absent.push({ text: p.text, price: null, confirmedAbsent: true });
  }
  if (known.length < 200) {
    return { promoted: false, reason: `Only ${known.length} posts have a known price — too few to train a price ranker.` };
  }

  // Split by POST before expanding into candidates, so candidates from one
  // post can never straddle the train/test boundary and inflate the score.
  ...
  const { rows } = buildPriceTrainingRows([...trainPosts, ...absent]);
  const vocab = new Map(), df = new Map();
  for (const r of rows) for (const f of new Set(r.feats)) df.set(f, (df.get(f) || 0) + 1);
  ...
  const X = rows.map(r => vec(r.feats));
  const y = rows.map(r => r.y);
  const model = trainLR(X, y, X.map((_, i) => i), mulberry32(17), vocab.size, 25, 0.5, 1e-5);

  // Held-out top-1 precision at the runtime threshold, mirroring predictPrice.
  let emitted = 0, correct = 0;
  for (const r of testPosts) {
    if (isShortStayListing(r.text)) continue;
    let best = null, bp = -1;
    for (const c of priceCandidates(r.text)) {
      const pr = predictRow(model, vec(priceFeatures(r.text, c)));
      if (pr > bp) { bp = pr; best = c; }
    }
    if (!best || bp < PRICE_CONFIDENCE) continue;
    emitted++; if (best.v === r.price) correct++;
  }
  const score = emitted ? correct / emitted : 0;

  const weights = {};
  for (const [f, i] of vocab) if (Math.abs(model.w[i]) > 1e-3) weights[f] = +model.w[i].toFixed(4);
  return {
    score, emitted,
    payload: {
      meta: {
        ...
        absent_posts: absent.length,
        human_labeled: fromHuman,
        heldout_precision: +score.toFixed(4),
...
function trainRoommatesHead(posts, rng = mulberry32(43)) {
  const rows = [];
  let fromHuman = 0;
  for (const p of rentalPosts(posts)) {
    const vt = verdictTruth(p, 'roommates');
    let y;
    if (typeof vt === 'boolean') { y = vt; fromHuman++; }
    else if (typeof p.tags_human_override?.roommates === 'boolean') { y = p.tags_human_override.roommates; fromHuman++; }
    else y = (regexExtractTags(p.text) || {}).roommates;
    rows.push({ masked: maskRoommateKeywords(p.text), y: y ? 1 : 0 });
  }
  if (rows.length < 200) {
    return { promoted: false, reason: `Only ${rows.length} rental posts available — too few to train the roommates head.` };
  }

  const order = rows.map((_, i) => i).map(i => [rng(), i]).sort((a, b) => a[0] - b[0]).map(r => r[1]);
  const cut = Math.floor(order.length * 0.8);
  const trainIdx = order.slice(0, cut), testIdx = order.slice(cut);

  const { vocab, idf, X } = buildCorpus(rows.map(r => r.masked));
  const y = rows.map(r => r.y);
  const model = trainLR(X, y, trainIdx, mulberry32(23), vocab.size);
  const score = testIdx.filter(i => (predictRow(model, X[i]) >= 0.5 ? 1 : 0) === y[i]).length / testIdx.length;

  // Final model trains on everything; the held-out score above is what gates it.
  const full = trainLR(X, y, X.map((_, i) => i), mulberry32(23), vocab.size);
  return {
    score,
    payload: {
      meta: {
        ...
        human_labeled: fromHuman,
        heldout_accuracy: +score.toFixed(4),
        masked: true,
      },
      bias: +full.b.toFixed(5),
      weights: exportHead(full, vocab, idf, 8000),
    },
  };
}

/**
 * Retrain both shadow heads and promote each independently — one regressing
 * must not block the other. Never touches tags; these heads stay shadow-only
 * whatever they score.
 */
export async function retrainShadowHeads(posts, onProgress = () => {}) {
  const out = {};
  const heads = [
    ['price',     PRICE_STORAGE_KEY,     PRICE_BASELINE,     trainPriceHead,     'heldout_precision'],
    ['roommates', ROOMMATES_STORAGE_KEY, ROOMMATES_BASELINE, trainRoommatesHead, 'heldout_accuracy'],
  ];
  for (const [name, key, baseline, train, metaKey] of heads) {
    try {
      onProgress(`Training ${name} head…`);
      const res = train(posts);
      if (!res.payload) { out[name] = { promoted: false, reason: res.reason }; continue; }

      const stored = (await chrome.storage.local.get(key))[key];
      const prev = stored?.meta?.[metaKey];
      // Bar can rise with a genuinely better model but never ratchet below the
      // shipped baseline — the same floor rule the label head uses.
      const floor = Math.max(baseline, prev ?? baseline) - SHADOW_GATE_EPSILON;
      if (res.score < floor) {
        out[name] = { promoted: false, score: res.score, floor, reason:
          `${name}: held-out ${(res.score * 100).toFixed(1)}% is below the gate ` +
          `(${(floor * 100).toFixed(1)}%) — keeping the current weights.` };
        continue;
      }
      await chrome.storage.local.set({ [key]: res.payload });
      out[name] = { promoted: true, score: res.score, floor, previous: prev ?? baseline,
                    human_labeled: res.payload.meta.human_labeled };
    } catch (err) {
      out[name] = { promoted: false, reason: `${name}: ${err.message || err}` };
    }
  }
  return out;
}
...
export async function runRetrain(onProgress = () => {}) {
  ...
  // Shadow heads ride the same button. Deliberately AFTER the label head and
  // in its own try/catch: these are experimental and must never be able to
  // fail — or block — the classifier retrain that actually affects tagging.
  let shadow = null;
  try {
    shadow = await retrainShadowHeads(posts, onProgress);
  } catch (err) {
    console.warn('[TLV Rentals] Shadow head retrain failed (classifier unaffected):', err);
  }
